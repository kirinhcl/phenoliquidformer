#!/bin/bash
#SBATCH --job-name=timothy-phenology
...
#SBATCH --output=logs/phenology_%j.out
#SBATCH --error=logs/phenology_%j.err
...
echo "=== Phenology Liquid NN: 48-fold LOPO on exp01 ==="
singularity exec $SIF python3 scripts/train_liquid_teacher.py \
    --config configs/timothy.yaml \
    --experiment exp01 \
    --model-type phenology \
    --output-dir results/phenology_teacher
