#!/usr/bin/env python3
"""Visualize temporal attention weights from trained PhenologyLiquidModel.

Loads the fold-0 checkpoint from results/phenology_teacher/exp01/fold_0/best_model_state.pt,
...
paper/figures/final/fig_attention_weights.{png,pdf}.
"""
...
CHECKPOINT_PATH = PROJECT_ROOT / "results/phenology_teacher/exp01/fold_0/best_model_state.pt"
...
def load_model(checkpoint_path: Path, cfg) -> PhenologyLiquidModel:
...
def make_figure(mean_attn: np.ndarray, das_values: list[int]) -> plt.Figure:
    ...
    # Normalise to the actual data range so subtle differences are visible
    vmin = float(mean_attn.min())
    vmax = float(mean_attn.max())
    # Guard against perfectly flat attention (expand range slightly for rendering)
    if vmax - vmin < 1e-6:
        vmin = vmin - 1e-4
        vmax = vmax + 1e-4
    ...
    ax_heat.set_title(
        "Temporal attention weights per head (exp01, N=48, averaged)",
        fontsize=8,
        pad=4,
    )

    # Remove imshow box spines
    ...
    # Annotate DAS 40 threshold on heatmap
...
def main() -> None:
    ...
    # Inject liquid sub-config if absent (mirrors train_liquid_teacher.py)
    if OmegaConf.select(cfg, "model.liquid", default=None) is None:
        cfg.model.liquid = OmegaConf.create(
            {"hidden_dim": 32, "modality_dim": 32, "n_layers": 1, "head_dim": 64}
        )
    ...
    png_path = OUT_DIR / "fig_attention_weights.png"
    pdf_path = OUT_DIR / "fig_attention_weights.pdf"
